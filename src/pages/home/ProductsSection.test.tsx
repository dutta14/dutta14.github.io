...
describe('ProductsSection', () => {
  ...
  it('renders the correct number of product cards', () => {
    const { container } = renderWithRouter(<ProductsSection />);
  ...
  it('renders "Read case study" links for each product', () => {
    renderWithRouter(<ProductsSection />);
    const links = screen.getAllByText(/Read case study/);
    expect(links.length).toBe(products.length);
    products.forEach(product => {
      const link = links.find(l => l.closest('.product-card')?.querySelector('h3')?.textContent === product.title);
      expect(link).toBeTruthy();
      expect(link?.closest('a')?.getAttribute('href')).toBe(`/case-study/${product.slug}`);
    });
  });
});
